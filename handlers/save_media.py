...
from handlers.helpers import str_to_b64, short_url
...
async def save_batch_media_in_channel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = f"https://telegram.me/{Config.BOT_USERNAME}?start=sdmoviesflix_{str_to_b64(str(SaveMessage.id))}"
...
async def save_media_in_channel(bot: Client, editable: Message, message: Message):
    try:
        ...
        await forwarded_msg.reply_text(
            ...
            disable_web_page_preview=True)
        share_link = f"https://telegram.me/{Config.BOT_USERNAME}?start=sdmoviesflix_{str_to_b64(file_er_id)}"
        short_link = get_short(share_link)
        await editable.edit(
